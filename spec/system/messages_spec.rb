...
RSpec.describe "Messages", type: :system do
  ...
  let!(:public_room) { create(:room, quest_id: public_quest.id) }
  ...
  describe "表示確認" do
    context "自身のメッセージの場合" do
      let!(:message) { create(:message, user: user, room_id: public_room.id, created_at: "2023-04-1 12:00:00") }
    ...
    context "他ユーザーのメッセージの場合" do
      let!(:other_message) { create(:message, user: other_user, room_id: public_room.id, created_at: "2023-04-1 12:00:00") }
