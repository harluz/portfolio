...
RSpec.describe "Tags", type: :request do
  ...
  describe "POST #create" do
    ...
    context "tagの作成に成功すること" do
      it "tagの数が増加していること" do
        expect do
          post quests_path, params: { quest_form: quest_add_tag_params }
        end.to change(Tag, :count).by(1)
      end

      it "tagが空である場合、tagの数が増加していないこと" do
        quest_add_tag_params[:name] = ""
        expect do
          post quests_path, params: { quest_form: quest_add_tag_params }
      ...
      it "追加されたtagの分だけ数が増加していること" do
        quest_add_tag_params[:name] = "旅行 食べ物"
        expect do
          post quests_path, params: { quest_form: quest_add_tag_params }
      ...
      it "tagが重複する場合、重複が除かれtagの数が増加していること" do
        quest_add_tag_params[:name] = "旅行 旅行"
        expect do
          post quests_path, params: { quest_form: quest_add_tag_params }
        end.to change(Tag, :count).by(1)
      end
  ...
  describe "PATCH #update" do
    ...
    it "新しいタグが追加された場合、tagの数が増加すること" do
      expect do
        patch quest_path(public_quest1), params: { quest_form: quest_change_tag_params }
      end.to change(Tag, :count).by(1)
    end

    it "既存のタグが追加された場合、tagの数が増加していないこと" do
      quest_change_tag_params[:name] = "sports"
      expect do
        patch quest_path(public_quest1), params: { quest_form: quest_change_tag_params }
      end.to change(Tag, :count).by(0)
    end
